...
@NamedQueries({
        ...
        @NamedQuery(name = "sumPersonCategoryByYearOrganizationMonthCategoryType", query = "select new com.noor.dto.PersonCategoryDTO (  " +
                ...
                "group by category.name,category.code,category.id,category.categoryType "),
        @NamedQuery(name = "sumDTOPersonAttendanceDetail",query = "select new com.noor.dto.PersonnelAttendanceDetailDTO(occupation.id,occupation.name," +
                "sum(detail.totalWorked),sum (detail.attendanceCount),sum (detail.totalHoursWorked),sum(detail.totalMinutesWorked),sum (detail.overtimeHoursWorked),sum (detail.overtimeMinutesWorked)) " +
